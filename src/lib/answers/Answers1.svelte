...
<p class = "answerkey"><br>
    a) The probability of no 5-star in 3 pulls is just losing a coin toss 3 times, except
    the coin has a {@html math("0.99")} chance of landing tails. So the answer is {@html math("0.99 \\times 0.99 \\times 0.99 = 0.970299")} .
</p>

<p class = "answerkey">The probability of no Ruan Mei in 2 pulls is to either:</p><br>
    <ul  class = "answerkey">
        <li>Get no 5-stars</li>
        <li>Get one 5-stars and lose the 50-50</li>
    </ul>
<p class = "answerkey"><br>
    b) There is one way to get no 5-stars: {@html math("0.99 \\times 0.99")} . <br>
    There are two ways to get 1 5-star and lose 50-50: either get the 5-star first or second.<br>
    Each event has a probability of {@html math("0.01 \\times 0.99 \\times 0.5")}<br>
    So the total probability to not get Ruan Mei is<br>
    {@html math("0.99 \\times 0.99 + 2 \\times 0.5 \\times 0.01 \\times 0.99")} which conveniently equals {@html math("0.99")} . 
</p>
...
<p class = "answerkey"><br>
    ...
    是否允許更改 is Silverwolf's Skill which causes {@html math("[60]")} toughness damage.<br>
    KafkaCNVA is Xu Hui which has {@html math("[5]")} letters (yeah... this wasn't a very good one).
</p>

<p class = "answerkey"><br>
    a) Himeko's damage on crit is {@html math("40000 \\times (1+150\\%) = 100000")} damage.
</p>

<p class = "answerkey"><br>
    b) The expected value can be found by using the following formula:<br>
    {@html math("E(X) = Pr(Crit) \\times DMG_{crit} + Pr(NoCrit) \\times DMG_{nocrit}")} . <br>
    So Himeko's damage is {@html math("0.6 \\times 100000 + 0.4 \\times 40000 = 76000")} . 
</p>
...
<p><br>Q3: <br></p>
<p class = "answerkey"><br>
    IPC is {@html math("[300]")} if you are NOT attempting to advance the achievement "Love and the Emperor". <br>
...
<p class = "answerkey"><br>
    You can either plug it into a graphing calculator like desmos.com,
    actually use calculus (god bless you),
    or just use the idea that 1:2 crit ratio is optimal (lol).
</p>

<p class = "answerkey"><br>
    Either way, you will get {@html math("x = 72.5")}, and the final damage number is {@html math("88050")} .
</p>
